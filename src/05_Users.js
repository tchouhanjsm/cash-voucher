...
function getUsers(token) {
  const owner = requirePermission_('users', token);
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    users.push({
      ...
      createdAt: formatDateTime_(data[i][8]),

      updatedAt: formatDateTime_(data[i][9]),
    });
  }

  writeAuditLog_(owner, 'VIEW_USERS', '', 'Viewed user management');
...
function addUser(token, userData) {
  ...
  sheet.appendRow([userId, email, name, role, true, pinHash, 0, '', now, now]);

  SpreadsheetApp.flush();

  writeAuditLog_(owner, 'ADD_USER', '', 'Added user ' + email + ' as ' + role);
...
function updateUser(token, userId, userData) {
  ...
  const oldPinHash = data[rowNumber - 1][5];

  const oldFailedAttempts = data[rowNumber - 1][6];

  const oldLockout = data[rowNumber - 1][7];

  const oldCreatedAt = data[rowNumber - 1][8];

  sheet
    .getRange(rowNumber, 2, 1, 9)
    .setValues([
      [
        email,
        name,
  ...
  writeAuditLog_(owner, 'UPDATE_USER', '', 'Updated user ' + email);
...
function changeMyPin(token, currentPin, newPin) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]) === String(user.userId)) {
      if (hashPin_(currentPin) !== String(data[i][5])) {
      ...
      writeAuditLog_(user, 'CHANGE_PIN', user.userId, 'PIN changed.');
